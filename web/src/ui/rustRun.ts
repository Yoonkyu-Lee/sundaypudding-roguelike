...
export function mountRustRun(app: HTMLElement, startSeed: number): void {
  ...
  let seed = startSeed;
  let appState: "title" | "hub" | "editor" | "jobsEditor" | "run" | "chardex" = "title";
  ...
  window.addEventListener("keydown", (e) => {
    ...
    if (appState === "jobsEditor") { if (e.key === "Escape" && !editing) { appState = "hub"; hubMode = "menu"; render(); } return; }
  ...
  window.addEventListener("contextmenu", (e) => { if (appState !== "editor" && appState !== "jobsEditor" && !(e.target as HTMLElement)?.closest("input,textarea")) e.preventDefault(); });
