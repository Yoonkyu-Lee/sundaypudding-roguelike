...
// 타겟은 엘리먼트(유닛 카드 또는 빈 앵커 칸) — 빈 칸 타겟팅에서도 화살표가 보이게.
export function drawArrow(app: HTMLElement, casterUid: string, targetEl: HTMLElement): void {
  const svg = app.querySelector<SVGSVGElement>(".arrows");
  const cEl = app.querySelector<HTMLElement>(`.card[data-uid="${casterUid}"]`);
  if (!svg || !cEl || !targetEl) return;
