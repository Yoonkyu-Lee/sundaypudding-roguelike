...
export function attachCamera(opts: CameraOpts): CameraControls {
  ...
  vp.addEventListener("pointerdown", (e) => { if (!canPan(e)) return; e.preventDefault(); panning = true; lastX = e.clientX; lastY = e.clientY; vp.setPointerCapture(e.pointerId); });
  ...
  vp.addEventListener("pointerup", () => { if (panning) { panning = false; onChange({ ...cam }); } });
