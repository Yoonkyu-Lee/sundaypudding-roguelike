// 맵 에디터 — 런 목록 화면(E1). repo+드래프트 카드, 새로만들기/테스트/내보내기/삭제.
// (단일 층 헥스 에디터 캔버스 = E2, 층 그래프 패널 = E3.)
import { esc } from "../battle/shared.ts";
import type { RunSource } from "./store.ts";

export interface EditorRunCard {
  id: string;
  name: string;
  source: RunSource;
  floors: number;
...
function card(r: EditorRunCard): string {
  ...
  const draftActions = r.source === "draft"
    ? `<button class="ed-btn" data-export="${r.id}">내보내기</button><button class="ed-btn ghost" data-del="${r.id}">삭제</button>`
    : "";
  return `<div class="ed-card">
    ...
    <div class="ed-card-actions">
      <button class="ed-btn"${r.valid ? "" : " disabled"} data-test="${r.id}">▶ 테스트플레이</button>
      ${draftActions}
    </div>
  </div>`;
}
