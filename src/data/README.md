...
런 하나 = 시작 파티 + **층(floor)의 선형 체인**. 각 층은 **헥스 인접 무방향그래프**(노드 + 맞닿은 헥스끼리의 무방향 변). 맵은 손으로 짠 JSON이 진실 — **타이틀 화면의 「🗺 맵 에디터」**로 드래그드롭 저작 → 「내보내기」로 `{id}.json` 받아 이 폴더(`src/data/runs/`)에 커밋하면 배포본이 된다. (JSON을 직접 손으로 써도 됨. 절차생성 `MapGenConfig`/`genMap`은 폐기)
...
규칙: ① 입장(`entryNodeId`)에서 **맞닿은 길**을 따라 **클리어 노드**에 도달하면 층 완료(보스는 강적이지만 길목). **재방문 불가**(지나온 칸 잠김 → 전진만). ② 갈림길로 보스/클리어 여러 개 → **아무 클리어든 진입하면 완료**. ③ 변은 **맞닿은 헥스끼리만**, 모든 노드는 입장과 연결되고 그 성분에 클리어가 있어야(엔진 `validateRun`이 저장 시 검증 — 비인접 변·고립 노드 거부).

**에디터 조작**: **광활한 육각 격자**(엑셀식) 어디든 카탈로그를 **드래그**해 배치, 배치된 노드는 **드래그로 이동**. 노드 **클릭**=선택 → **Del** 삭제(입장 제외). **인접하면 기본 연결**되며, 두 칸 사이 변에 **호버(점선 미리보기)→클릭(실선 벽)**으로 길을 막거나 다시 잇는다(연결됨=선 없음). 뷰포트 고정 — **휠=줌**, **휠(가운데) 드래그=카메라 이동**(우상단 ＋／－／리셋).
