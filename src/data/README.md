...
런 하나 = 시작 파티 + **층(floor) 그래프**. 각 층은 **헥스 인접 무방향그래프**(노드 + 맞닿은 헥스끼리의 무방향 변)이고, 층끼리는 **클리어 노드의 `toFloor`**로 이어진다(없으면 그 클리어=승리). `entryFloorId`가 시작 층. 맵은 손으로 짠 JSON이 진실 — **타이틀 화면의 「🗺 맵 에디터」**로 드래그드롭 저작 → 「내보내기」로 `{id}.json` 받아 이 폴더(`src/data/runs/`)에 커밋하면 배포본이 된다. (JSON을 직접 손으로 써도 됨. 절차생성 `MapGenConfig`/`genMap`은 폐기)
