...
**에디터 조작**: 카탈로그에서 노드를 **드래그**해 빈 헥스에 놓고, 배치된 노드는 **드래그로 이동**. 노드를 **클릭**해 선택 후 **Del**로 삭제(입장 노드 제외). **인접하면 기본 연결**되며, **변에 호버 → 클릭**으로 길을 막거나(벽) 다시 잇는다(연결됨은 선만 표시). 맵 뷰포트는 **휠=줌**, **휠(가운데 버튼) 드래그=이동**(우상단 ＋／－／리셋 버튼).
