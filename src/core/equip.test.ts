...
test("장착: 방어구 HP 보정 + 슬롯검증 + 교체/해제 인벤토리 왕복", () => {
  ...
  equipItem(run, "beef", "armor", "iron_plate");
...
test("장착: 무기 dmgFlat·치명/쉴드 보정이 전투 유닛에 합산", () => {
  ...
  const states = [
    { charId: "beef", pos: { row: 1, col: 0 }, hp: 40, maxHp: 40, skillDmgBonus: {}, activeSkillIds: ["gangta"], equipped: { weapon: "brass_knuckle", armor: "iron_plate" } },
  ];
  const g = createBattle(7, enc, states);
  const u = g.units.find((x) => x.side === "ally")!;

  assert.equal(u.equipDmgFlat, 4); // brass_knuckle dmgFlat
  assert.equal(u.critChance, CHARACTERS["beef"].critChance + 8); // brass_knuckle critChance +8
...
test("장착: 무장 없으면 보정 0 (기존 동작 보존)", () => {
  const enc = { id: "t", name: "t", allies: [], enemies: [{ charId: "thug", pos: { row: 0, col: 0 } }], boss: false };
  const states = [{ charId: "beef", pos: { row: 1, col: 0 }, hp: 40, maxHp: 40, skillDmgBonus: {}, activeSkillIds: ["gangta"], equipped: {} }];
  ...
  assert.equal(u.critChance, CHARACTERS["beef"].critChance);
