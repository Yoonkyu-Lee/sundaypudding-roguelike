...
const CARD_SIZE := Vector2(0.6, 1.0)
const FEET_Y := 0.01   # 발이 칸 타일 표면에 닿는 높이

func setup(color: Color) -> void:
	_build_card(color)

## 오버레이가 투영할 카드의 시각적 머리 월드좌표.
## 카드=풀 빌보드 → 화면-위(=카메라 up)로 뻗음. 발 + 카메라_up × 카드높이 = 실제 머리(월드-위로 띄우면 틸트에서 어긋남).
func head_world_pos(camera: Camera3D) -> Vector3:
	return global_position + Vector3(0, FEET_Y, 0) + camera.global_transform.basis.y * CARD_SIZE.y
