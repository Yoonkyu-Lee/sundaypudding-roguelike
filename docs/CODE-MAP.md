...
| `src/web/editor/` | web | **맵 에디터 GUI**(구조 에디터) — `store.ts`(드래프트 localStorage·repo 병합·blankRun·JSON 내보내기) · `ops.ts`(노드/변/층 순수 변이·`addNode`/`moveNode`(드래그 이동)·`autoConnectAdjacent`(인접=기본 연결)·`adjacentPairs`(벽)·`gridCells`) · `controller.ts`(목록↔편집 상태·핸들러) · `editorRender.ts`(목록) · `editView.ts`(엑셀식 광활한 육각 격자(메모이즈 SVG path)+고정 뷰포트 카메라(휠 줌·가운데드래그 팬)·픽셀→axial 드롭 배치/이동·클릭 선택(Del)·벽=공유 변 테두리(호버 점선/클릭 실선)·층 패널). `validateRun`/`hexAdjacent` 재사용 | `createEditor` · `renderEditor` |
