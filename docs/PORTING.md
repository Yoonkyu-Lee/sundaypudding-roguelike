...
- [x] **4-2 소킹 + 4-3 가지치기** (2026-06): 사용자 판단으로 소킹 단축 후 즉시 가지치기. **제거**: TS 엔진(`core/combat`·`ai`·`engine`·`observation`·`util`·`run/{run,layers,shop,encounter,rewards,items,passives,helpers,save,view}`)·`core/tests`(골든/differential 전부)·골든생성기(diff-corpus·ai-corpus·golden-update·stress-sweep)·CLI(`src/cli`)·TS 프론트 게임경로(`main.ts`→부팅만·`render.ts` renderApp/computeTgtFromState/buildSkillBar·`overlay.ts`·`handlers/`·`save.ts`·`coreAdapter`·`rustBattle`·`hub.makeRun`). **남김**: `core/types/*`(IPC 계약)·`core/rng.ts`(타입)·`core/run/{types,graph}.ts`(DTO+에디터 hex/validate)·`src/data/`·`export-data`. **이주**: `computeAreaCells`→`web/battle/areaGeo.ts`, `unlockedTier`→`web/meta.ts`(인라인). 게이트=typecheck+web test(29)+cargo test+vite build 통과.
> **✅ Phase 4 완료.** 엔진=Rust(`spr-core`) 단일. TS 골든 엔진은 `archive/ts-core`+`tag ts-golden-oracle`에 보관(재실행 가능). **주의**: 골든 재생성·differential은 이제 아카이브 체크아웃 필요 → 신규 메커니즘은 Rust 직접 + 회귀픽스처(`rust/spr-core/tests/*.generated.json` 동결) + 실플레이.
...
> **✅ 전 게임로직 마이그레이션 완료 (2026-06).** 전투+AI+런 오케스트레이션이 모두 Rust로 이식·**바이트 검증**(풀 런 differential: 맵·전투·보상RNG·상점·인카운터·런패시브·성장·층전환). 풀 게임 세션 API(`RunSession`) + Tauri 커맨드 + `?core=rust&full=1` 하네스로 **전체 로그라이크가 Rust 코어로 플레이됨**. **save 왕복(영속화)까지 완료** → Phase 0~3 + 영속화 전부 끝. 남은 것 = TS 코어 은퇴(검증 종료 결정, 사용자 몫).
...
- [~] P2-7 view + save:
  ...
  - [x] **save 왕복** — RunState(전투 GameState 포함) 전 트리 serde(Serialize+Deserialize). `run/save.rs`(serialize_run/deserialize_run) + `RunSession::save_json/load_json`(데이터맵 재로드·charId 호환검사) + `run_save`/`run_load` 커맨드 + `rustRun.ts` localStorage 영속(전용 키 `spr_rust_save_v1`, 부팅 복원·이어하기). **GameState.log은 `#[serde(skip)]`**(append-only·복원 시 빈 벡터 → GameEvent serde 트리 회피, 전투 중 저장 안전). 검증=**라운드트립 결정론**(mid-battle 세이브→로드 후 이어진 이벤트 델타 바이트 동일 — RNG·유닛·상태·룰 카운터 복원). 게임로직 불변(엔진 프리미티브 추가 없음)
