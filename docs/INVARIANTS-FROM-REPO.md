...
> `campaign` = 캠페인 종료 시 / `action` = step 입력 검증 시.
>
...
| ID | 심각도 | 규칙 | 근거 | 시점 |
|---|---|---|---|---|
| J1 | CRIT | 모든 캠페인은 **유한 스텝 내** won/lost 도달(교착 없음) | `autoRun` guard(400)/bg(600) 패턴 | campaign |
| J2 | CRIT | **같은 시드 + 같은 행동 시퀀스 → 동일 이벤트 로그**(자기 일치) | Rng(seed) 단일 출처, Math.random 금지 | campaign |
| J3 | CRIT | 무작위 선택도 시드 결정적: 같은 캠페인 시드 → 같은 선택열 | 캠페인 러너가 state.rng(또는 별도 시드 rng) 사용 | campaign |
| J4 | CRIT | 캠페인 전체에서 크래시/패닉/throw **0건** | (측정 지표) | campaign |
...
6. **검사 시점**: step / round / event / campaign 매핑이 일치하는가?
...
- **AI 정책(chooseAction)의 결정론** — 무작위 캠페인과 별개로, AI 경로도 시드 결정적인지.
