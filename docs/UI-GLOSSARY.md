...
## 맵/런 도메인 용어 (SoT) — 표시·주석·문서 통일

> 코드 식별자는 영어 유지(`RunDef`·`FloorDef`·`MapNode`·`start`·`clear`·`toFloor`·`entryFloorId`·`MapEdge`). **플레이어 표시·UI 문구·주석·문서는 아래 한글 용어**로 통일.
...
| **캐릭터 도감** | 해금 캐릭터·전직 트리·스킬 열람(허브 메뉴 진입). **구성·세부 = 아래 「캐릭터 도감 화면」 절** | `codex.ts` `renderCodex` | `.codex` |
...
허브 진입점 `📖 캐릭터 도감`. **전체 = `codex.ts` `renderCodex` / `.codex`**(풀폭, 전체 페이지 스크롤 없음 — 패널 내부 스크롤만). 2열: **좌=디테일**, **우=캐릭터 목록**.
...
| **정체성 바** | 좌측 상단 **고정 패널**(스크롤 안 함) — 아바타·이름 · 능력치 · **기본 특성 칩**(내재·상시) | `codex.ts` `baseTraitsBar` | `.cdx-idbar` · `.cdx-basetraits`/`.cdx-bt-chip` |
...
| └ **직업 노드** | 그 차수 직업. **분기는 세로 스택**(⑂ 표식). 노드 안에 **부여 특성 카드**(✦ 이름+설명) — 분기 차별점 | `codex.ts` `jobsByTier`·`traitCard` | `.cdx-jobcard` · `.cdx-tcard` |
...
| └ **강화 토글** | 강화 체인 스킬은 **차수 토글(T1/T2…)** 한 카드에서 원본↔강화 스펙 전환(공간 절약, 로컬 DOM·재렌더 없음) | `codex.ts` `skillCardEl` | `.cdx-upg` · `.cdx-tabs`/`.cdx-tbtn`/`.cdx-tbody` |
