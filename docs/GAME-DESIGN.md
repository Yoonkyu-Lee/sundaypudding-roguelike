...
| 필드 분리(2026-06) | 옛 구현은 `Skill.tier`를 숙련도 게이트로 겸용 → **`masteryReq` 필드 분리**(강화 tier와 독립) + `classReq`(전직). 필드 선언=S1 ✓. **게이트 재배선**(`rewards.rs`/`shop.rs` tier→masteryReq)은 S4 | ✓ 필드 S1 / 게이트 ☐ S4 |
...
| 전직 효과 | ① 캐릭에 **패시브(`TraitDef`) 추가**(런 한정) ② 그 차수(`classReq`) **전용 스킬이 보상 풀에 편입**(해금) | ✓ ① S2 (상태+패시브 적용) / ☐ ② S4 |
